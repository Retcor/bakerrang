...
export const TOOL_DEFINITIONS = Object.freeze([
  { id: 'storybook', name: 'Story Book', shortName: 'Story Book', accent: 'var(--accent-story)', legacyPath: '/storybook', liveUrl: null, envKey: 'VITE_STORYBOOK_URL' },
...
export const LAUNCHER_DEFINITION = Object.freeze({
  id: 'launcher',
  name: 'All tools — Launcher',
  liveUrl: 'https://launch.bakerrang.com',
  envKey: 'VITE_LAUNCHER_URL'
})
...
export const resolveDestinations = (env = {}, legacyBase = 'https://bakerrang.com') => {
  ...
  const withUrl = (definition) => ({
    ...definition,
    url: env[definition.envKey] || definition.liveUrl || `${base}${definition.legacyPath}`
  })
  return {
    tools: TOOL_DEFINITIONS.map(withUrl),
    account: withUrl(ACCOUNT_DEFINITION),
    launcher: withUrl(LAUNCHER_DEFINITION)
  }
}
  ...
  return (
    <div className='br-theme-control' role='group' aria-label='Theme'>
      {['light', 'dark', 'system'].map((value) => (
        <button key={value} type='button' data-theme={value} title={`${value[0].toUpperCase()}${value.slice(1)} theme`} aria-label={`${value[0].toUpperCase()}${value.slice(1)} theme`} aria-pressed={preference === value} onClick={handleThemeClick}>
...
const LauncherIcon = () => <svg viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='1.75' strokeLinecap='round' strokeLinejoin='round' aria-hidden='true'><path d='M19 12H5M11 6l-6 6 6 6' /></svg>
const SignOutIcon = () => <svg viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='1.7' strokeLinecap='round' strokeLinejoin='round' aria-hidden='true'><path d='M15 4H6a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h9' /><path d='M18 15l3-3-3-3M10 12h11' /></svg>
...
export const BrandLink = ({ logoSrc, launcherUrl = '/', appName, appUrl = '/' }) => (
  <div className='br-product-brand'>
    <a className='br-brand br-brand--mark' href={launcherUrl} aria-label='BakerRang — all tools'>
      <img className='br-brand__mark' src={logoSrc} alt='' />
      {!appName && <span className='br-brand__word'>BakerRang</span>}
    </a>
    {appName && <><span className='br-brand__rule' aria-hidden='true' /><a className='br-brand' href={appUrl}><span className='br-brand__word'>{appName}</span></a></>}
  </div>
)

export const AppSwitcher = ({ destinations, current, showLauncher = true }) => {
  const menu = usePopover()
  return (
    <div className={`br-menu ${menu.open ? 'is-open' : ''}`} ref={menu.ref}>
      <IconButton aria-label='Switch app' aria-haspopup='true' aria-expanded={menu.open} onClick={() => menu.setOpen(!menu.open)}><GridIcon /></IconButton>
      <div className='br-popover br-switcher' role='menu' aria-label='Your tools'>
        ...
        <div className='br-switcher__grid'>
          {destinations.tools.map((tool) => (
            <a key={tool.id} className={`br-switcher__cell ${tool.id === current ? 'is-current' : ''}`} role='menuitem' href={tool.url} style={{ '--accent': tool.accent }} aria-current={tool.id === current ? 'page' : undefined}>
...
export const AccountMenu = ({ destinations, themeControl = false }) => {
  ...
  return (
    <div className={`br-menu ${menu.open ? 'is-open' : ''}`} ref={menu.ref}>
      <button className='br-avatar' aria-label='Account menu' aria-haspopup='true' aria-expanded={menu.open} onClick={() => menu.setOpen(!menu.open)}>{initials(auth.user)}</button>
      <div className='br-popover' role='menu' aria-label='Account'>
        <div className='br-profile-summary'><strong>{auth.user?.displayName || 'BakerRang user'}</strong><span>{auth.user?.email}</span></div>
  ...
  return (
    <header className='br-topbar'>
      <div className='br-wrap br-topbar__inner'>
        <BrandLink logoSrc={logoSrc} launcherUrl={homeUrl} />
        ...
        <div className='br-topbar__right'>
          ...
          {!loading && !authenticated && <Button variant='gold' onClick={auth.login}><GoogleIcon /><span className='br-signin-full'>Sign in with Google</span><span className='br-signin-short'>Sign in</span></Button>}
          {authenticated && <><AppSwitcher destinations={destinations} showLauncher={false} /><AccountMenu destinations={destinations} /></>}
        </div>
      </div>
    </header>
  )
...
export const AppFooter = ({ logoSrc, year = new Date().getFullYear() }) => (
  <footer className='br-footer'>
    <div className='br-wrap br-footer__inner'>
      <a className='br-brand' href='/' aria-label='BakerRang home'><img className='br-brand__mark' src={logoSrc} alt='' /><span className='br-brand__word'>BakerRang</span></a>
