...
const sourcePath = path.join(repoRoot, 'web/.impeccable/mocks/polyglot-icon.svg')
...
const run = async () => {
  ...
  const embedded = source.replace(/<!--[\s\S]*?-->/g, '').replaceAll('bakerrang-logo.png', `data:image/png;base64,${logo.toString('base64')}`)
  await fs.mkdir(publicPath, { recursive: true })
  for (const [size, name] of [[512, 'android-chrome-512x512.png'], [192, 'android-chrome-192x192.png'], [180, 'apple-touch-icon.png'], [32, 'favicon-32x32.png'], [16, 'favicon-16x16.png']]) {
