  ...
  return (
    <div className={`min-h-screen flex items-center justify-center theme-bg ${isDark ? 'dark-theme-bg' : 'light-theme-bg'} relative overflow-hidden`}>
      ...
      <div className='absolute inset-0'>
        <div className={`absolute top-1/4 left-1/4 w-72 h-72 rounded-full opacity-20 blur-3xl ${isDark ? 'bg-amber-500' : 'bg-amber-200'}`} />
        <div className={`absolute bottom-1/4 right-1/4 w-96 h-96 rounded-full opacity-20 blur-3xl ${isDark ? 'bg-yellow-600' : 'bg-yellow-200'}`} />
        <div className={`absolute top-1/2 left-1/2 w-64 h-64 rounded-full opacity-10 blur-3xl ${isDark ? 'bg-orange-400' : 'bg-orange-200'} transform -translate-x-1/2 -translate-y-1/2`} />
      </div>
