...
function NavContent ({ contextNav, onNavigate, onNavigateRequest, pathname }: { contextNav?: ContextNav[], onNavigate?: () => void, onNavigateRequest?: (href: string) => boolean, pathname: string }) {
  const follow = (event: MouseEvent<HTMLAnchorElement>, href: string) => {
    if (onNavigateRequest && !onNavigateRequest(href)) { event.preventDefault(); return }
    onNavigate?.()
  }
  ...
  return (
    <div className="min-h-screen bg-bg lg:grid lg:grid-cols-[17rem_minmax(0,1fr)]">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-68 flex-col bg-sidebar p-5 lg:flex">
        <Link className="mb-8 rounded-md focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-focus" href="/" onClick={(event) => { if (onNavigateRequest && !onNavigateRequest('/')) event.preventDefault() }}><Brand /></Link>
        <NavContent contextNav={contextNav} onNavigateRequest={onNavigateRequest} pathname={pathname} />
        {account}
      </aside>
      ...
      {drawerOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div aria-label="Navigation menu" aria-modal="true" className="relative flex h-full w-[min(20rem,88vw)] flex-col bg-sidebar p-5 shadow-md" id="mobile-navigation" ref={drawerRef} role="dialog">
            ...
            <NavContent contextNav={contextNav} onNavigate={() => setDrawerOpen(false)} onNavigateRequest={onNavigateRequest} pathname={pathname} />
            {account}
          </div>
        </div>
      )}
