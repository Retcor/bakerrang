...
export function SitePreviewFrame ({ onPageSelected, onSectionSelected, pageId, site }: {
  ...
  return (
    <section aria-label="Website preview" className="mb-5 min-w-0 overflow-hidden rounded-lg border border-border bg-bg p-2">
      <div className="min-w-0 overflow-auto" data-site-preview-viewport="">
        <iframe className="block h-[38rem] w-full border-0 bg-white" onLoad={() => { readySourceRef.current = null; sentRef.current = null }} ref={frameRef} src={SITE_PREVIEW_FRAME_ROUTE} title="Website preview" />
      </div>
    </section>
  )
}
