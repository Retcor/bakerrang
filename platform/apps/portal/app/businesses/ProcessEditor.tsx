...
import { findHomePage, isProcessSection, type SiteDefinition } from '@bakerrang/site-schema'
import { Button, Input, Textarea } from '@bakerrang/ui'
import { ApiError } from '../../lib/api'
import { upsertHomeProcess } from '../../lib/site'
import { RowActions } from './RowActions'
import { WebsiteEditorShell } from './WebsiteEditorShell'

type Row = { key: string, id?: string, title: string, description: string }
export function ProcessEditor ({ sectionId, tenantId, site, onCancel, onDirtyChange = () => {}, onSaved }: { sectionId: string, tenantId: string, site: SiteDefinition, onCancel: () => void, onDirtyChange?: (dirty: boolean) => void, onSaved: (site: SiteDefinition) => void }) {
  const section = findHomePage(site)?.sections.find((item) => item.id === sectionId)
