...
import { addableSectionTypes } from './sectionDefinitions'
...
export function AddSectionDialog ({ onAdded, onClose, onRefresh, open, pageId, site, tenantId }: {
  ...
  return <Dialog description="Add a section to this page. The new section opens in its exact editor after it is created." onClose={onClose} open={open} title="Add section">
    <div className="space-y-5">
      {groups.map((group) => <section key={group}><h3 className="text-sm font-semibold text-fg">{group}</h3><div className="mt-2 space-y-3">{choices.filter(({ definition }) => definition.group === group).map(({ definition, disabled: unavailable, reason, type }) => {
        const hoursBlocked = type === 'businessHours' && reason === 'Set a weekly schedule in Site setup first'
        const disabled = unavailable || Boolean(pending)
