...
import { SegmentedControl, ToggleRow } from './WebsiteToolPrimitives'
...
export function HeaderEditor ({ chrome, onBack, onCancel, onControllerChange, onDirtyChange = () => {}, onLivePreview, onPreview, onSaved, site, tenantId }: {
  chrome?: 'card' | 'rail'
  onBack?: () => void
  onCancel: () => void
  onControllerChange?: (controller: ActiveEditorController | null) => void
  onDirtyChange?: (dirty: boolean) => void
  onLivePreview?: (patch: Pick<SiteDefinition, 'header'>) => void
  onPreview: () => void
  ...
  const header = useMemo<SiteHeader>(() => ({ brandDisplay, navigation: { items: normalizeNavigationItems(items) }, ...(ctaEnabled ? { cta: { buttonLabel: buttonLabel.trim(), action: { type: actionType, value: actionValue.trim() } } } : {}) }), [actionType, actionValue, brandDisplay, buttonLabel, ctaEnabled, items])
  ...
  useEffect(() => {
    onLivePreview?.({ header })
  }, [header, onLivePreview])
  ...
  return <WebsiteEditorShell chrome={chrome} dirtyValue={header} editor="header" error={error} onBack={onBack} onCancel={onCancel} onControllerChange={onControllerChange} onDirtyChange={onDirtyChange} onSubmit={(event) => void submit(event)} saveDisabled={!dirty || ctaInvalid} saving={saving} secondaryActions={<Button disabled={saving || dirty} onClick={onPreview} type="button" variant="secondary">Preview Home</Button>}>
    <fieldset><legend className="text-sm font-semibold text-fg">Brand display</legend><div className="mt-3"><SegmentedControl ariaLabel="Brand display" disabled={saving} onChange={setBrandDisplay} options={[{ value: 'logo', label: 'Logo' }, { value: 'logoAndName', label: 'Logo + name' }, { value: 'name', label: 'Name' }]} value={brandDisplay} /></div></fieldset>
