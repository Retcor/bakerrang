...
export function BusinessWorkspace ({ children, description, presentation = 'page', tenantId, title }: { children: ReactNode, description: string, presentation?: 'page' | 'builder', tenantId: string, title: string }) {
  ...
  return (
    <BusinessNavigationGuardContext.Provider value={reportDirty}>
      <AppShell contentClassName={presentation === 'builder' ? 'min-w-0 bg-bg lg:h-screen lg:overflow-hidden' : undefined} contextNav={nav} onNavigateRequest={requestNavigation}>
        {presentation === 'page' && <><Link className="mb-5 inline-flex min-h-10 items-center text-sm font-semibold text-fg-muted hover:text-fg focus-visible:outline-2 focus-visible:outline-focus" href="/" onClick={(event) => followLink(event, '/')}>← All businesses</Link><PageHeader description={description} eyebrow={business?.name ?? 'Business workspace'} title={title} actions={business && <Badge tone={business.status === 'ACTIVE' ? 'success' : 'neutral'}>{business.status}</Badge>} /></>}
...
export function BusinessOverview ({ tenantId }: { tenantId: string }) {
  ...
  const areas = [
    ...
    { href: `${base}/settings`, title: 'Settings', description: 'Export business data or permanently delete this business.', action: 'Manage data & lifecycle →' }
  ]
