...
describe('Social Profiles editor', () => {
  ...
  it('loads ordered links, excludes configured platforms from other selectors, and is mobile-safe', () => {
    ...
    expect(screen.getByRole('button', { name: 'Move Instagram up' })).toBeDisabled()
    expect(screen.getByRole('button', { name: 'Move Facebook down' })).toBeDisabled()
  })
  ...
  it('edits, reorders, removes, and saves ordered canonical links through the focused endpoint', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Move Facebook up' }))
    fireEvent.click(screen.getByRole('button', { name: 'Remove Facebook' }))
